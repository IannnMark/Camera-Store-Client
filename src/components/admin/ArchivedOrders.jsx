...
export default function ArchivedOrders() {
  ...
  const fetchOrders = async () => {
    try {
      const res = await fetch(`${apiUrl}/admin/soft-deleted-order`, {
        method: "GET",
        credentials: "include",
      });
  ...
  const handleDeleteOrder = async (orderId) => {
    ...
    if (confirmDelete) {
      try {
        const res = await fetch(`${apiUrl}/admin/order/delete/${orderId}`, {
          method: "DELETE",
          credentials: "include",
        });
        const data = await res.json();
    ...
    if (confirmDelete) {
      try {
        const res = await fetch(
          `${apiUrl}/admin/order/restore-order/${orderId}`,
          {
            method: "PUT",
            credentials: "include",
          }
      ...
      cell: (row) => (
        <>
          <button
            ...
            onClick={() => handleRestoreOrder(row._id)}
          >
